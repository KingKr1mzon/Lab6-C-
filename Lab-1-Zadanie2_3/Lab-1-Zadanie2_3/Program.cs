...
namespace Lab1
{
    class Program
    {
        private static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nКопия первого кошелька: " +
                wallet1Copy.ToString());

            Money result = wallet1.Subtract(wallet2);

            Console.WriteLine("\nРезультат вычитания:");
            Console.WriteLine(wallet1.ToString() + " - " +
                wallet2.ToString() + " = " + result.ToString());

            Console.WriteLine("\n--- Задание 3. Перегрузка операций ---");

            Money mInc = new Money(wallet1);
            mInc++;
            Console.WriteLine("wallet1++ (добавили копейку): " +
                mInc.ToString());

            Money mDec = new Money(wallet1);
            mDec--;
            Console.WriteLine("wallet1-- (убрали копейку): " +
                mDec.ToString());

            uint rublesOnly = wallet1;
            double kopeksOnly = (double)wallet1;
            Console.WriteLine("\nНеявный uint (рубли): " + rublesOnly);
            Console.WriteLine("Явный double (копейки как дробь): " +
                kopeksOnly);

            Console.WriteLine("\nБинарное вычитание (wallet1 - wallet2): " +
                (wallet1 - wallet2).ToString());

            uint scalar = ReadUint("\nВведите число для вычитания из wallet1: ");
            Console.WriteLine("wallet1 - uint: " +
                (wallet1 - scalar).ToString());
            Console.WriteLine("uint - wallet1: " +
                (scalar - wallet1).ToString());
            ...
            while (true)
            {
                ...
                else
                {
                    Console.WriteLine("Ошибка. " +
                        "Введите целое положительное число (uint).");
                }
            }
        }
        ...
        private static byte ReadByte(string prompt)
        {
            ...
            while (true)
            {
                ...
                {
                    Console.WriteLine("Ошибка. " +
                        "Введите число от 0 до 255 (byte).");
                }
            }
        }
    }
